    ...
    public void onCommand(CommandArgs command) {
        ...
        for (Player player : Bukkit.getOnlinePlayers()) {
            ...
            if (silent && profile != null && profile.getRank().isAboveOrEqual(Rank.TRIAL_MOD)) {
                player.sendMessage(ChatColor.RED + "(Punishment) " + ChatColor.GREEN + target.getName() + " has been kicked by " + sender.getName() + " for " + reason + ".");
            }
            else {
                player.sendMessage(ChatColor.GREEN + target.getName() + " has been kicked by " + sender.getName() + " for " + reason + ".");
            }
        }
