 ...
 * <p>
 * The implementation is slightly different than in RFC 4648. During encoding, padding is not added, and during decoding the last incomplete chunk is not taken into account. The result is that
 * multiple strings decode to the same byte array, for example, string of sixteen 7s ("7...7") and seventeen 7s both decode to the same byte array. TODO(sarvar): Revisit this encoding and whether this
...
public class Base32 {
  ...
  public static class DecodingException extends Exception {
    private static final long serialVersionUID = -5424598882580431068L;
  ...
  protected byte[] decodeInternal(String encoded) throws DecodingException {
    ...
    final int outLength = encodedLength * SHIFT / 8;
  ...
  protected String encodeInternal(byte[] data) {
    ...
    final int outputLength = (data.length * 8 + SHIFT - 1) / SHIFT;
    final StringBuilder result = new StringBuilder(outputLength);
    ...
    while (bitsLeft > 0 || next < data.length) {
      if (bitsLeft < SHIFT)
        if (next < data.length) {
          ...
          final int pad = SHIFT - bitsLeft;
          buffer <<= pad;
          bitsLeft += pad;
        }
      final int index = MASK & (buffer >> (bitsLeft - SHIFT));
      bitsLeft -= SHIFT;
